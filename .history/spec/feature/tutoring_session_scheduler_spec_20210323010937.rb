...
RSpec.describe TutoringSessionScheduler, type: :feature do
  ...
  describe 'SHOW' do
    let!(:tsession) {TutoringSession.create(scheduled_datetime: scheduled_datetime,
                                        tutor_id: tutor.id)}
    let(:tsession2) {TutoringSession.create(scheduled_datetime: scheduled_datetime + 1.week,
                                         tutor_id: tutor.id)}
    before do
      tsession.users << tutor
    end
