...
RSpec.describe TutoringSessionExportHelper, type: :helper do
  ...
  let(:csv_table) {CSV.read(Rails.root.join('spec/helpers/tutoring_hours_spec.csv'), headers: true)}
  # let(:csv_string) { nil }
  ...
  context 'when tutor is indeed a tutor' do
    let!(:tutor_role) { tutor.roles << Role.create(role_name: 'Tutor') }
  ...
  context 'when tutor is not actually a tutor' do
    it 'does not show them in the csv' do
      ...
      csv_table = CSV.read(Rails.root.join('spec/helpers/tutoring_hours_spec_3.csv'), headers: true)
