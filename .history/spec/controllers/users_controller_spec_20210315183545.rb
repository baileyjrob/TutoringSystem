  ...
  describe 'POST delete_schedule' do
    context 'when user is signed in' do
      ...
      before do
        ...
        sign_in user
        current_user.tutoring_sessions << [tutoring_session1, tutoring_session2, tutoring_session3]
      end
      ...
      it 'deletes' do
        delete_session_path(tutoring_session2)
        expect(current_user.tutoring_sessions).to eq([tutoring_session1, tutoring_session3])
      end
      it 'does not delete the session object' do
        delete_session_path(tutoring_session2)
        expect(tutoring_session2).to exist
      end
    end
  end
end
