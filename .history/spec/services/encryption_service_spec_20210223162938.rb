  ...
  #describe 'Encryption and Decryption' do
    ...
    context 'involving user data' do
      ...
      it 'still finds correct user with encrypted data' do
        @user.save
        expect(User.where("first_name = John").order("created_on DESC").find(1)).to eq(@user)
        expect(User.where("last_name = Doe").order("created_on DESC").find(1)).to eq(@user)
        expect(User.where("email = johndoe@tamu.edu").order("created_on DESC").find(1)).to eq(@user)
        @user.destroy
      end
    end
