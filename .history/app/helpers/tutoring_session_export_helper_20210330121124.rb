...
module TutoringSessionExportHelper
  ...
  # Switches to a new tutor when a change is detected
  def get_new_tutor(writer, hours_worked, entry, current_tutor)
    ...
    CSV.open(file, 'w', write_headers: true, headers: headers) do |writer|
      table.each do |entry|
        ...
        if current_tutor != full_name(entry)
          get_new_tutor(writer, hours_worked, entry, current_tutor)
        end
        # Update hours_worked
